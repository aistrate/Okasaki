...
module Ex03_10b (module Set, RedBlackSet(..), Color(..),
...
rbalance a x (T R (T R b y c) z d) = T R (T B a x b) y (T B c z d)
  ...
  insertSet x s = T B a y b
    where ins E = T R E x E
          ins s@(T R a y b) =
            ...
            else if x > y then T R a y (ins b)
                 else s
          ins s@(T B a y b) =
            if x < y then lbalance (ins a) y b
            else if x > y then rbalance a y (ins b)
                 else s
