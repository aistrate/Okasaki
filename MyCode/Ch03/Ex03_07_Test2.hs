...
hunitTests = TestList [
  "insert/findMin" ~:
  [
    findMin (insert 'e' (empty::ExplicitMin LeftistHeap Char)) ~?= 'e',
    findMin (insert 'f' (insert 'e' (empty::ExplicitMin LeftistHeap Char))) ~?= 'e',
    findMin (insert 'e' (insert 'f' (empty::ExplicitMin LeftistHeap Char))) ~?= 'e'
  ],
...
testHeap' :: Ord a => ExplicitMin LeftistHeap a -> [a] -> Bool
