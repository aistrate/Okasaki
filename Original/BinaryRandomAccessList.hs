...
module BinaryRandomAccessList (module RandomAccessList, BinaryList) where
  ...
  unconsTree [] = error "BinaryRandomAccessList: empty list"
  unconsTree [One t] = (t, [])
  ...
  instance RandomAccessList BinaryList where
    ...
    lookup i (BL ts) = look i ts
      where
        fail = error "BinaryRandomAccessList.lookup: bad subscript"
    ...
    update i y (BL ts) = BL (upd i ts)
      where
        fail = error "BinaryRandomAccessList.update: bad subscript"
